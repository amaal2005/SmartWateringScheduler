...
public class GardenPanel extends JPanel {
    ...
    // Minimum pixel distance between two plant centres to avoid overlap
    private static final int MIN_PIXEL_DISTANCE = PLANT_SIZE + 8;
    ...
    protected void paintComponent(Graphics g) {
        ...
        drawPlants(g2);
        drawPlacementCounter(g2);
        drawBottomHint(g2);
    }

    // ── mouse handlers ────────────────────────────────────────────────────────

    /** Left-click: place selected plant or show info of existing plant. */
    private void handleGardenClick(int pixelX, int pixelY) {
        ...
        if (selectedPlant.isPlaced()) {
            JOptionPane.showMessageDialog(this,
                    "This plant is already placed.\nRight-click it to remove it first.",
                    "Already Placed",
                    JOptionPane.INFORMATION_MESSAGE);
            return;
        }
        ...
        // ── overlap check ─────────────────────────────────────────────────
        if (isTooCloseToExistingPlant(pixelX, pixelY)) {
            JOptionPane.showMessageDialog(this,
        ...
        // ── boundary check (keep inside the grid area) ────────────────────
        if (pixelY < HEADER_HEIGHT + 4 || pixelX < OFFSET + 4) {
            if (outputArea != null)
                outputArea.append("⚠ Click inside the garden area.\n");
            return;
        }
    ...
    /** Right-click: remove plant from garden (un-place it). */
    private void handleRightClick(int pixelX, int pixelY) {
        if (plants == null || plants.isEmpty()) return;

        Plant target = findPlantAt(pixelX, pixelY);
        if (target == null) return;

        int confirm = JOptionPane.showConfirmDialog(this,
                "Remove " + target.getPlantTypeName() + " from the garden?",
                "Remove Plant",
        ...
        if (confirm == JOptionPane.YES_OPTION) {
            ...
            if (outputArea != null)
                outputArea.append("✗ Removed " + target.getPlantTypeName() + " from the garden.\n");
            plantListPanel.refreshList();
            repaint();
        }
    }
    ...
    /**
     * Returns true if the candidate pixel position is too close to any
     * already-placed plant (within MIN_PIXEL_DISTANCE pixels).
     */
    private boolean isTooCloseToExistingPlant(int pixelX, int pixelY) {
        for (Plant plant : plants) {
            if (!plant.isPlaced()) continue;

            int px = (int) (plant.getX() * SCALE_FACTOR) + OFFSET  + PLANT_SIZE / 2;
            int py = (int) (plant.getY() * SCALE_FACTOR) + HEADER_HEIGHT + PLANT_SIZE / 2;

            double dist = Math.hypot(pixelX - px, pixelY - py);
            if (dist < MIN_PIXEL_DISTANCE) return true;
        }
        return false;
    }
    ...
    private Plant findPlantAt(int pixelX, int pixelY) {
        for (Plant plant : plants) {
            ...
            if (new Rectangle(x, y, PLANT_SIZE, PLANT_SIZE).contains(pixelX, pixelY))
                return plant;
        }
        return null;
    }
    ...
    private void showPlantInfo(Plant plant) {
        String status = switch (plant.getPredictedNeedsWater()) {
            case  1 -> "Needs water 💧";
            case  0 -> "OK — no water needed ✓";
            default -> "Not predicted yet";
        };
        ...
        String info = "Plant " + getPlantIndex(plant)
                ...
                + "\nPredicted: "      + status
                + "\nPosition: ("
                + String.format(java.util.Locale.US, "%.1f", plant.getX()) + ", "
                + String.format(java.util.Locale.US, "%.1f", plant.getY()) + ")"
                + "\n\n(Right-click the plant on the garden to remove it)";
    ...
    private int getPlantIndex(Plant plant) {
        for (int i = 0; i < plants.size(); i++)
            if (plants.get(i) == plant) return i + 1;
        return 0;
    }
    ...
    private void drawPlants(Graphics2D g2) {
        ...
        for (int i = 0; i < plants.size(); i++) {
            ...
            Color fillColor = switch (plant.getPredictedNeedsWater()) {
                case  1 -> dangerRed;
                case  0 -> okGreen;
            ...
            g2.drawString(typeShort,
                    x + (PLANT_SIZE - fm.stringWidth(typeShort)) / 2,
                    y + (PLANT_SIZE - fm.getHeight()) / 2 + fm.getAscent() - 1);

            // Index label below circle
            g2.setColor(textGray);
            g2.setFont(new Font("SansSerif", Font.PLAIN, 12));
            g2.drawString("P" + (i + 1), x + 6, y + PLANT_SIZE + 14);

            // Coordinates below index  ← NEW
            String coords = "(" + String.format(java.util.Locale.US, "%.0f", plant.getX())
                    + "," + String.format(java.util.Locale.US, "%.0f", plant.getY()) + ")";
            g2.setFont(new Font("SansSerif", Font.PLAIN, 10));
            g2.setColor(new Color(110, 110, 110));
            g2.drawString(coords, x - 2, y + PLANT_SIZE + 26);

            g2.setFont(new Font("SansSerif", Font.BOLD, 16));
        }
    }

    /** Counter in the top-right corner showing placed / total. */
    private void drawPlacementCounter(Graphics2D g2) {
        if (plants == null || plants.isEmpty()) return;

        long placed = plants.stream().filter(Plant::isPlaced).count();
        int total   = plants.size();

        String text = "Placed: " + placed + " / " + total;
        g2.setFont(new Font("SansSerif", Font.PLAIN, 13));
        FontMetrics fm = g2.getFontMetrics();
        int tw = fm.stringWidth(text);

        int rx = getWidth() - tw - 28, ry = HEADER_HEIGHT + 10;
        g2.setColor(new Color(240, 245, 238, 220));
        g2.fillRoundRect(rx - 8, ry - 14, tw + 16, 22, 10, 10);
        g2.setColor(new Color(160, 190, 140));
        g2.setStroke(new BasicStroke(1f));
        g2.drawRoundRect(rx - 8, ry - 14, tw + 16, 22, 10, 10);

        g2.setColor(new Color(60, 100, 40));
        g2.drawString(text, rx, ry);
    }

    private void drawRandomPath(Graphics2D g2) {
        if (randomPath == null || randomPath.size() < 2) return;
        g2.setColor(randomGray);
        g2.setStroke(new BasicStroke(2f));
        for (int i = 0; i < randomPath.size() - 1; i++) {
            Plant p1 = randomPath.get(i), p2 = randomPath.get(i + 1);
            g2.drawLine(
                    (int)(p1.getX()*SCALE_FACTOR)+OFFSET+PLANT_SIZE/2,
                    (int)(p1.getY()*SCALE_FACTOR)+HEADER_HEIGHT+PLANT_SIZE/2,
                    (int)(p2.getX()*SCALE_FACTOR)+OFFSET+PLANT_SIZE/2,
                    (int)(p2.getY()*SCALE_FACTOR)+HEADER_HEIGHT+PLANT_SIZE/2);
        }
    }

    private void drawOptimizedPath(Graphics2D g2) {
        if (optimizedPath == null || optimizedPath.size() < 2) return;
        g2.setColor(pathBlue);
        g2.setStroke(new BasicStroke(3f, BasicStroke.CAP_ROUND, BasicStroke.JOIN_ROUND,
                0, new float[]{8f, 6f}, 0));
        for (int i = 0; i < optimizedPath.size() - 1; i++) {
            Plant p1 = optimizedPath.get(i), p2 = optimizedPath.get(i + 1);
            g2.drawLine(
                    (int)(p1.getX()*SCALE_FACTOR)+OFFSET+PLANT_SIZE/2,
                    (int)(p1.getY()*SCALE_FACTOR)+HEADER_HEIGHT+PLANT_SIZE/2,
                    (int)(p2.getX()*SCALE_FACTOR)+OFFSET+PLANT_SIZE/2,
    ...
    private void drawBottomHint(Graphics2D g2) {
        ...
        g2.drawString("left-click → place / info   |   right-click → remove",
                getWidth() - 340, getHeight() - 10);
    }
}